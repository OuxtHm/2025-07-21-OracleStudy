...
public class BoardDAO {
	...
	// 메모리 공간을 한개만 생성 -> 메모리 누수현상을 방지 = 싱글톤
	...
	{
		try {
			Class.forName("oracle.jdbc.dirver.OralceDriver"); // 리플렉션 -> 클래스 이름으로 변수, 메소드, 생성자를 제어할 수 있게 만든 프로그램(MyBatis / Spring)
			// oracle.jdbc.dirver.OralceDriver -> DriverManager가 관리
		} catch (Exception e) {}
	}
	// 2. 싱글톤 -> 반드시 static : 메모리 공간이 한 개만 생성
	/*
		가비지 컬렉션 : 사용하지 않거나 / null인 상태의 객체의 메모리 회수
	...
	*/
	...
	public List<BoardVO> boardListData(int page)
	{
		...
		try
		{
			...
			// 2. 오라클로 전송할 SQL 문장
			String sql = "SELECT no, subject, name, TO_CHAR(regdate, 'YYYY-MM-DD') as regdate, hit, num "
	...
	{
		BoardVO vo = new BoardVO();	// 게시물 1개에 대한 모든 정보
