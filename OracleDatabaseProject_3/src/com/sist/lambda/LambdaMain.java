...
public class LambdaMain {
	...
	*/
	public void lambdaDisplay1()
	{
		...
		list.stream()
			.filter(vo -> vo.getSal() >= 3000)	// WHERE절 -> Predicate
	...
	// SELECT empno, ename, job, hiredate, sal, FORM emp;
	...
	public void lambdaDisplay3()
	{
		...
		list.stream()
			//.filter(vo -> vo.getEname().startsWith("A"))	//	A로 시작하는 문장
			//.filter(vo -> vo.getEname().endsWith("T"))	// T로 끝나는 문장
			.filter(vo -> vo.getEname().contains("T"))		// T를 포함하는 문장 
			.forEach(vo -> System.out.println(
					vo.getEmpno() + " "
					+ vo.getEname() + " "
					+ vo.getHiredate() + " "
	...
	public void lambdaDisplay4()
	{
		...
		list.stream()
			...
			.ifPresent(vo -> System.out.println(vo.getEname() + " " +vo.getHiredate()));
	}
