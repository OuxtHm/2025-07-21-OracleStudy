...
public class EmpDAO {
	// 1. 공통 사용된 객체
	private Connection conn; // 오라클 연결 객체 -> 인터페이스로 구성
	...
	public List<EmpVO> empListData() 
	{
		...
		try {
			...
			// 4. 실행 후에 결과 값을 메모리에 저장 요청
			ResultSet rs = ps.executeQuery();
			while (rs.next()) // 첫번째 줄 부터 데이터 읽기
			{
				EmpVO vo = new EmpVO();
	...
	public List<EmpVO> empFindData(String ename) {
		...
		try {
			...
			// 2. 오라클에 어떤 문장을 전송할 지
			// 오라클은 데이터가 대소문가 구분 -> EMP에 모든 데이터가 대문자로 이루어져있다.
